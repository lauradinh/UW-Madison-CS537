...
/* IN THIS VERSION, ADDED THE LAST ELSE STATMENT TO WAKEUP1 AND COMMENTED ONE OF THE TICK INCREMENTS. STILL ISN'T PASSING ALL THE TESTS ALL THE TIME(TILL NOW TEST 12 AND 14) BUT AT LEAST PASSES USUALLY 13/14 TESTS*/
// test 14 fails when just the p->ticks++ exists and test 12 fails when just the 2 lined incrementer exists
...
*hold_lottery(int total_tickets) 
{
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->boosts_left > 0){
        ...
        //p->boosts_left--;                       // CHANGED to decrementing after returning to scheduler
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // decrement boosts counter
    for(cur_p = ptable.proc; cur_p < &ptable.proc[NPROC]; cur_p++){    // CHANGED: decrement the boosts after returning the winning process
      if(cur_p->state != RUNNABLE)
        continue;
      else{
        if(cur_p->boosts_left > 0){
          cur_p->boosts_left--;
        }
      }
    }
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  p->chan = chan;
  p->state = SLEEPING;
  //p->boosts_left++;                             // CHANGED JUST A TEST
  p->sleep_flag = 0;
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING && chan == p->chan){
      ...
      else if(chan == &ticks && p->tick_sleep == p->sleep_flag) {
        p->state = RUNNABLE;
      }
      else{
        // cprintf("Working\n");
