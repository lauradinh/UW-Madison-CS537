    ...
    Reducer reduce, int num_reducers, Partitioner partition)
{
    ...
    // go through each element in the linked list add a partition
    struct node* curr = head;
    while(curr != NULL) {
        curr->data->partition = (*partition)(curr->data->key, num_reducers);
        curr = curr->next;
    }
    sort_partition();
    sort_key();

    // may have to change this global counter to work with threads
    ...
    while (kvl_counter < size()) {
        ...
        if(pair != NULL) {
            ...
            pthread_create(&reducethreads[i], NULL, (void *)reduce, (void *)arg);
